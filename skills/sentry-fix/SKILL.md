...
The pipeline has 12 steps. Each step has a hard gate — if a gate fails, the pipeline stops and records the failure reason.

### Step 1: Load State

`.sentry-autofix/state.json`을 읽는다. 파일이 없으면 "초기 설정이 필요합니다. `/sentry-setup`을 먼저 실행하세요."를 출력하고 중단한다.
...
비어있지 않으면 즉시 전체 중단: "워킹 트리에 미커밋 변경이 있습니다. 커밋하거나 stash한 뒤 다시 실행하세요."

clean이면 baseBranch로 이동한다:
```bash
git checkout <config.baseBranch>
git pull origin <config.baseBranch>
```

추가로 아래 중 하나라도 해당하면 즉시 중단한다:
...
### Step 4.5: Sentry 이슈 상세 조회

서브에이전트의 분석 JSON은 요약이다. 재현 테스트의 정확도를 높이기 위해 메인 파이프라인이 직접 Sentry MCP로 이슈 상세를 조회한다.

**조회 대상:**
- 최신 이벤트의 전체 스택트레이스 (서브에이전트 요약이 아닌 원본)
- request context: URL, HTTP method, headers, body (있으면)
- breadcrumbs: 오류 직전 사용자 행동 흐름
- tags: device, os, browser, release 등
- 사용자 컨텍스트: user ID, email, IP 등 (있으면)

**이 데이터를 Step 5에서 활용한다:**
- request body/params → 테스트의 입력값으로 사용
- breadcrumbs → 재현 시나리오의 순서 참고
- tags → 특정 환경에서만 발생하는지 확인

Sentry MCP 도구를 사용하여 이슈 ID로 최신 이벤트를 조회한다.
...
- Sentry 이벤트의 실제 입력값(request params, user context 등)을 테스트 데이터로 활용한다
