...
`.sentry-autofix/state.json`을 읽는다.

**자동 정리:** `processed`에서 `pr_created`, `skipped`, `failed` 상태이고 `processedAt`이 30일 이상 경과한 항목을 삭제한다. `.sentry-autofix/logs/`에서 30일 지난 로그 파일도 삭제한다.

파일이 없으면 아래 기본값으로 초기화한다.
...
- 정확히 일치하는 브랜치가 없으면 `git branch -a`에서 입력값을 포함하는 브랜치를 검색한다
  - 유사 브랜치가 1개 이상 있으면: "`<입력값>` 브랜치가 없습니다. 혹시 이 브랜치를 의미하셨나요?" + 후보 목록을 보여주고 선택하게 한다
  - 유사 브랜치도 없으면: "해당 브랜치가 존재하지 않습니다. 다시 입력해주세요."를 출력하고 재입력을 요청한다
