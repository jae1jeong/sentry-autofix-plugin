...
초기화 시 `package.json`의 `scripts`에서 test/lint/typecheck 명령을 자동 탐지하여 config에 채운다:
- `scripts.test` → `testCommand`
- `scripts.lint` → `lintCommand`
- `scripts.typecheck` 또는 `scripts.type-check` → `typeCheckCommand` (없으면 `npx tsc --noEmit` 시도)
